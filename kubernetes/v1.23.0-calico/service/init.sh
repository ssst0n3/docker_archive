...
log() {
  echo "[docker-archive/kubernetes/v1.23.0-calico] $1" >> /dev/kmsg
}
...
install_calico_network() {
  ...
  kubectl create -f https://projectcalico.docs.tigera.io/archive/v3.22/manifests/tigera-operator.yaml >>/dev/kmsg 2>&1
  kubectl create -f https://projectcalico.docs.tigera.io/archive/v3.22/manifests/custom-resources.yaml >>/dev/kmsg 2>&1
