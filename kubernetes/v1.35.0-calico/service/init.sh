...
log() {
  echo "[docker-archive/kubernetes/v1.35.0-calico] $1" >> /dev/kmsg
}
...
install_calico_network() {
  ...
  # https://docs.tigera.io/calico/3.31/getting-started/kubernetes/requirements
  # https://docs.tigera.io/calico/3.31/getting-started/kubernetes/quickstart#install-calico
  ...
  helm install calico projectcalico/tigera-operator --version v3.31.3 --namespace tigera-operator >>/dev/kmsg 2>&1
