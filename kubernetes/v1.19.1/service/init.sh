...
log() {
  echo "[docker-archive/kubernetes/v1.19.1] $1" >> /dev/kmsg
}

hostnamectl set-hostname kubernetes-1-19-1

log "kubeadm init"
kubeadm init --skip-phases=preflight --config=/kind/kubeadm.conf --v=6 > /dev/kmsg 2>&1
log "taint"
